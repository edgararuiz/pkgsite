...
In its current state, `pkgsite`'s functions build on top of each other in
...
arguments should work without errors. Internally, the function applies these
defaults when specific arguments are left `NULL`.

For the index page, `pkgsite` by default will group based on the 'family' tag. 
Any documentation that does not have a 'family' tag will be listed first. This
functionality can be overridden by configuring a custom grouping specified via 
